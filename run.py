...
if __name__ == '__main__':
    if not os.path.exists(data_dir):
        print("\033[33m{}\033[0m".format('data set not find!'))
        exit(127)
    ...
    for info in testInfo:
        fail_point = None
        print('test for {}'.format(info))
        for point in testInfo[info]:
            file_path = '{}/{}/{}'.format(data_dir, info, point)
            if os.path.exists(file_path + '.in') and os.path.exists(file_path + '.out'):
                ...
                print("\033[31m{}\033[0m".format('data set is broken at {}!'.format(file.path)))
                exit(127)
        clean()
        if fail_point == None:
            print("\033[36m{}\033[0m".format('success in {}'.format(info['name'], fail_point)))
        else:
            print("\033[31m{}\033[0m".format('case {} fail at {}'.format(len(errorCase), len(testInfo))))
